...
| naturalSortWhenEmpty   | boolean           | false    | `false`          | When a collection's `sort` is empty, send the entity's schema `natural_sort` to the server as the default order (request-only; the `sort` model stays empty).            |
...
| filterEditingLocation  | string            | false    | `'query-builder'` | Default `QkitSearch` location for the filter builder (`'query-builder'` or `'none'`). Overridable per `QkitSearch` via the prop of the same name.                       |
| fieldsEditingLocation  | string            | false    | `'none'`          | Default `QkitSearch` location for the fields editor (`'query-builder'`, `'collection'`, or `'none'`). Overridable per `QkitSearch`.                                      |
| sortEditingLocation    | string            | false    | `'collection-column'` | Default `QkitSearch` location for sort editing (`'query-builder'`, `'collection-modal'`, `'collection-column'`, or `'none'`). Overridable per `QkitSearch`.          |
...
The three `*EditingLocation` options (`filterEditingLocation`, `fieldsEditingLocation`, `sortEditingLocation`) configure `QkitSearch` only — they set where it exposes its filter/fields/sort builders (only `QkitSearch` has a query builder). A standalone `QkitCollection` ignores them: it takes its sort UI from its own `sortEditingLocation` prop (default `'collection-column'`, i.e. sortable column headers) and its fields editor from `editFields`.
