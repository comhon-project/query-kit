export default {
  query_builder: 'constructor de consultas',
  filter: 'filtro',
  filters: 'filtros',
  add: 'agregar',
  add_filter: 'agregar un filtro',
  add_value: 'agregar un valor',
  delete: 'borrar',
  remove: 'eliminar',
  search: 'buscar',
  reset: 'reiniciar',
  validate: 'validar',
  confirm: 'confirmar',
  close: 'cerrar',
  collapse: 'contraer',
  expand: 'expandir',
  like: 'contiene',
  not_like: 'no contiene',
  ilike: 'contiene (A=a)',
  not_ilike: 'no contiene (A=a)',
  ends_with: 'termina con',
  doesnt_end_with: 'no termina con',
  iends_with: 'termina con (A=a)',
  idoesnt_end_with: 'no termina con (A=a)',
  begins_with: 'comienza con',
  doesnt_begin_with: 'no comienza con',
  ibegins_with: 'comienza con (A=a)',
  idoesnt_begin_with: 'no comienza con (A=a)',
  in: 'en',
  not_in: 'no en',
  null: 'nulo',
  not_null: 'no nulo',
  contains: 'incluye',
  not_contains: 'no incluye',
  and: 'y',
  or: 'o',
  has: 'tiene',
  has_not: 'no tiene',
  at_least: 'al menos',
  at_most: 'como máximo',
  exactly: 'exactamente',
  yes: 'si',
  no: 'no',
  condition: 'condición',
  entity_condition: 'condición de parentesco',
  group: 'grupo',
  operator: 'operador',
  export: 'exportar',
  columns: 'columnas',
  fields: 'campos',
  sort: 'orden',
  pagination: 'paginación',
  previous: 'anterior',
  next: 'siguiente',
  results: 'resultados',
  choose_condition_element: 'elige sobre qué elemento quieres aplicar la condición',
  infinite_list: 'lista infinita',
  paginated_list: 'lista paginada',
  asc: 'asc.',
  desc: 'desc.',
  unsorted: 'sin ordenar',
  page: 'página',
  boolean_choice: 'opción booleana',
  collection: 'colección',

  go_to_collection: 'ir a los resultados de búsqueda',
  go_to_query_builder: 'ir al constructor de consultas',
  undo: 'deshacer',
  redo: 'rehacer',
  invalid_field: 'columna inválida:',
  invalid_property: 'propiedad inválida:',
  invalid_scope: 'alcance inválido:',
  invalid_operator: 'operador inválido:',
  invalid_entity: 'entidad inválida:',
  invalid_type: 'tipo inválido:',
  invalid_filter: 'filtro no válido, no se pueden mostrar los resultados',
  entities: 'entidades',
  reorder: 'reordenar',
  item_grabbed: 'agarrado, use las flechas para mover, Espacio para soltar',
  item_moved: 'movido a la posición',
  item_dropped: 'soltado',
  reorder_cancelled: 'reorden cancelado',
};
