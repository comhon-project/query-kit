export default {
  query_builder: 'query builder',
  filter: 'filter',
  filters: 'filters',
  add: 'add',
  add_filter: 'add a filter',
  add_value: 'add a value',
  delete: 'delete',
  remove: 'remove',
  search: 'search',
  reset: 'reset',
  validate: 'validate',
  confirm: 'confirm',
  close: 'close',
  collapse: 'collapse',
  expand: 'expand',
  like: 'contains',
  not_like: "doesn't contain",
  ilike: 'contains (A=a)',
  not_ilike: "doesn't contain (A=a)",
  ends_with: 'ends with',
  doesnt_end_with: "doesn't end with",
  iends_with: 'ends with (A=a)',
  idoesnt_end_with: "doesn't end with (A=a)",
  begins_with: 'begins with',
  doesnt_begin_with: "doesn't begin with",
  ibegins_with: 'begins with (A=a)',
  idoesnt_begin_with: "doesn't begin with (A=a)",
  in: 'in',
  not_in: 'not in',
  null: 'null',
  not_null: 'not null',
  contains: 'includes',
  not_contains: "doesn't include",
  and: 'and',
  or: 'or',
  has: 'has',
  has_not: "doesn't have",
  at_least: 'at least',
  at_most: 'at most',
  exactly: 'exactly',
  yes: 'yes',
  no: 'no',
  condition: 'condition',
  entity_condition: 'relationship condition',
  group: 'group',
  operator: 'operator',
  export: 'export',
  columns: 'columns',
  fields: 'fields',
  sort: 'sort',
  pagination: 'pagination',
  previous: 'previous',
  next: 'next',
  results: 'results',
  choose_condition_element: 'choose on which element you want to apply the condition',
  infinite_list: 'infinite list',
  paginated_list: 'paginated list',
  asc: 'asc.',
  desc: 'desc.',
  unsorted: 'unsorted',
  page: 'page',
  boolean_choice: 'boolean choice',
  collection: 'collection',

  go_to_collection: 'go to search results',
  go_to_query_builder: 'go to query builder',
  undo: 'undo',
  redo: 'redo',
  invalid_field: 'invalid column:',
  invalid_property: 'invalid property:',
  invalid_scope: 'invalid scope:',
  invalid_operator: 'invalid operator:',
  invalid_entity: 'invalid entity:',
  invalid_type: 'invalid type:',
  invalid_filter: 'invalid filter, results cannot be displayed',
  entities: 'entities',
  reorder: 'reorder',
  item_grabbed: 'grabbed, use arrows to move, Space to drop',
  item_moved: 'moved to position',
  item_dropped: 'dropped',
  reorder_cancelled: 'reorder cancelled',
};
