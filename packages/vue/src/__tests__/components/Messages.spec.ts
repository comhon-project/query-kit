...
describe('Messages', () => {
  describe('InvalidData', () => {
    it('renders translated message key and value', () => {
      const wrapper = mount(InvalidData, {
        props: { messageKey: 'invalid_field', value: 'foo' },
      });
  ...
  describe('InvalidField', () => {
    it('passes field to InvalidData with invalid_field key', () => {
      const wrapper = mount(InvalidField, { props: { field: 'my_col' } });
      expect(wrapper.text()).toContain('invalid column:');
  ...
  describe('locale reactivity', () => {
    it('updates InvalidData translation when locale changes', async () => {
      ...
      loadedTranslations['fr'] = { invalid_field: 'colonne invalide :' };
      locale.value = 'fr';
      await nextTick();
