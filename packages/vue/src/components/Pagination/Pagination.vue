...
<template>
  <nav :aria-label="translate('pagination')">
    <ul :class="classes.pagination">
      ...
      <li v-for="index in nearPages" :key="index" :class="index == currentPage ? classes.active : ''" @click="(e) => updatePage(index, e)">
        <a href="#">{{ index }}</a>
      </li>
      <li v-if="currentPage < (count - 3)">...</li>
      <li v-if="count && count != 1" :class="count == currentPage ? classes.active : ''" @click="(e) => updatePage(count, e)">
        <a href="#">{{ count }}</a>
      </li>
      <li @click="(e) => updatePage(currentPage + 1, e)">
