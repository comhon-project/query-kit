<script setup>
  ...
  watch(
    isUsingShortcuts, 
    () => isUsingShortcuts.value ? firstShortcut.value.focus() : null,
    { flush: 'post' }
  )
</script>
...
<template>
  <div v-if="filteredActions.length">
    ...
    <template v-if="isUsingShortcuts">
        <button ref="firstShortcut" @click="$emit(filteredActions[0].action)" @focusout="finishUsingShortcuts" class="qkit-shortcut qkit-focus-only" :class="classes.btn">{{ translate(filteredActions[0].translation) }}</button>
        <button v-for="index in (filteredActions.length - 1)" @click="$emit(filteredActions[index].action)" @focusout="finishUsingShortcuts" class="qkit-shortcut qkit-focus-only" :class="classes.btn">{{ translate(filteredActions[index].translation) }}</button>
    </template>
  </div>
</template>
